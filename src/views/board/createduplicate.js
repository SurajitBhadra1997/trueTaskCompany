...
export default class boardAddModal extends Component {
 ...
 loadtagsOption = async () =>
 {
   ...
   cur_data=this.state.projectlist.filter(
    ...
    let temp=[];
    let temp1=[];
    temp1=cur_data[0].setting.length>0?cur_data[0].setting[0].project_tags.split(','):null;
    if(temp1!=null)
    temp1.forEach((element) => {
      let dom ={ label:element,value:element };
      temp.push(dom);
...
this.state.obj2=data;
 ...
 getStatus= async (id) => {
   ...
   if (result && result.data ) {
    let str=result.data.project_status;
    let arr=str.split(',');
    arr.forEach(element => {
      let dom = { label:element,value:element };
      data_cat.push(dom);
    ...
    createDuplicate = async () =>
  {
    ...
    if (validation) {
      ...
      let data = new FormData();
       ...
       data.append("originsl_task_id",this.state.task_id);
       data.append("addnew","duplicate");
    ...
    this.props.duplicateCreated(true);
      } else {
        this.props.duplicateCreated(false);
      }
    ...
    return (
      <div>
        ...
        <div
          ...
          id="duplicate-new-task-modal"
          tabIndex={-1}
          role="dialog"
          aria-labelledby="duplicate-new-task-modalLabel"
          aria-hidden="true"
        >
          <div className="modal-dialog modal-dialog-centered modal-lg">
            <div className="modal-content">
              <div className="modal-header">
                <h4 className="modal-title" id="duplicate-new-task-modalLabel">
                  Create New Duplicate Task
                </h4>
                <button
                  type="button"
                  className="close"
