...
export default class addSubTask extends Component {
  ...
  {
    if(nextProps.true_subtask_id!="")
    {
      console.log("ghj");
      setTimeout(() => {
    this.setState({
      subtask_id:nextProps.true_subtask_id,
  ...
  deleteImage = async ()=>
  {
    ...
    console.log(data);
    let result = await HttpClient.requestData("del-subtask-img", "POST", data);
  ...
  getTaskList = async () =>
 {
  let data = {
   ...
   if (result && result.status) 
     {
        let image=result.data.attachment?result.data.attachment:"";
          // let preview=this.state.image_url+image;
           let estimate=result.data.effort_estimate?result.data.effort_estimate:"";
    ...
    effort:est1,
    description:result.data&&result.data.task_description!="null"?result.data.task_description:"",
    startDate:result.data.planned_start_date,
    endDate:result.data.planned_end_date,
    assignedOn:result.data.assigned_on,
    selectedFiles:image!=""?[
      {
...
//    });
   let mem=result.data.assigned_to?result.data.assigned_to:"";
   console.log(mem);
   let status=result.data.status?result.data.status:"";
   let priority=result.data.project_priority?result.data.project_priority:"";
   let tags=result.data.tags?result.data.tags:"";
  if(mem!="")
  {
...
// console.log("cur_data",cur_data);
 ...
 loadpriorityOption = async () =>
 {
     console.log(this.state.projectlist);
   let cur_data=[];
   console.log('bal', this.state.project_id);
   cur_data=this.state.projectlist.filter(
     
    (item) => item.auto_id == this.state.project_id
    );
    console.log(cur_data);
    let temp=[];
    let temp1=[];
   ...
   cur_data=this.state.projectlist.filter(
    ...
    cur_data[0].staff_get.forEach((element) => {
      ...
      temp.push(dom);
         ...
         this.getStatus(this.state.project_id);
 }
    ...
    editSubTask = async () =>
    {
      ...
      if (validation) {
        ...
        let data = new FormData();
         data.append("linked_project",this.state.project_id);
         data.append("subtask_id",this.state.subtask_id);
        ...
        if (result && result.status) {
         ...
         this.setState({
            type: "success",
            status: true,
            errMsg: " sub task Edited successfully",
    ...
    return (
     ...
     />
      ...
      <div
        ...
        tabIndex={-1}
        role="dialog"
        aria-labelledby="edit-sub-task-modalLabel"
        aria-hidden="true"
      >
          <LoadingOverlay
        ...
        <div className="modal-dialog modal-dialog-centered modal-lg">
          <div className="modal-content">
            <div className="modal-header">
              <h4 className="modal-title" id="edit-sub-task-modalLabel">
                Edit Sub-Task
              </h4>
              <button
                type="button"
